...
def _prepare_source(df: pd.DataFrame, raw_col: str, pair_source: str) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(
            columns=["date", "amount_ils", "raw_text", "pair_source", "date_key", "amount_key"]
        )
    ...
    prepared = pd.DataFrame(
        {
            ...
            "raw_text": _series_or_default(df, raw_col).astype("string").fillna(""),
            "pair_source": pair_source,
        }
    )
    prepared["date_key"] = prepared["date"]
    prepared["amount_key"] = prepared["amount_ils"]
    return prepared.dropna(subset=["date_key", "amount_key"])


def _prepare_ynab(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(columns=["date_key", "amount_key", "ynab_payee_raw", "ynab_category_raw"])
...
def _join_pairs(source_df: pd.DataFrame, ynab_df: pd.DataFrame) -> pd.DataFrame:
    ...
    joined = source_df.merge(ynab_df, on=["date_key", "amount_key"], how="inner")
    return joined[
        [
            "date",
            "amount_ils",
...
def match_pairs(bank_df: pd.DataFrame, card_df: pd.DataFrame, ynab_df: pd.DataFrame) -> pd.DataFrame:
    ...
    bank_pairs = _join_pairs(_prepare_source(bank_df, "description_raw", "bank-ynab"), ynab_prepared)
    card_pairs = _join_pairs(_prepare_source(card_df, "description_raw", "card-ynab"), ynab_prepared)
    ...
    key_counts = (
        pairs.groupby(["date_key", "amount_key"], dropna=False).size().rename("_key_count").reset_index()
    )
    pairs = pairs.merge(key_counts, on=["date_key", "amount_key"], how="left")
