...
def _load_source_inputs(paths: list[Path]) -> pl.DataFrame:
    ...
    for path in paths:
        ...
        if string_cols:
            df = df.with_columns(
                [pl.col(name).fill_null("").alias(name) for name in string_cols]
            )
        fp = (
        ...
        if (fp == "").all():
            warnings.warn(
                f"Skipping {path} (all fingerprint values empty).", UserWarning
            )
...
def _dedupe_source_overlaps(source_df: pl.DataFrame) -> pl.DataFrame:
    ...
    card_card_suffix = pl.coalesce(
        [text("source_account"), text("account_name"), pl.lit("")]
    ).map_elements(
    ...
    card = (
        work.filter((pl.col("_source_norm") == "card") & valid)
        .select("_row_index", *key_cols)
        .with_columns(pl.int_range(0, pl.len()).over(key_cols).alias("_dup_rank"))
    )

    direct_drop = (
        card.join(
            bank.with_columns(pl.lit(True).alias("_matched")),
    ...
    secondary_card = (
        ...
        .select("_row_index", *second_key_cols)
        .with_columns(
    ...
    drop_rows = (
        pl.concat([direct_drop, secondary_drop], how="vertical_relaxed")
        .unique()
        .sort("_row_index")
    )
    ...
    return (
        work.join(
            drop_rows.with_columns(pl.lit(True).alias("_drop")),
            on="_row_index",
            how="left",
        )
...
def _build_options_from_applied(
    ...
    if applied.empty:
        return pd.DataFrame(
            columns=["payee_options", "category_options"], index=applied.index
        )
    ...
    for rule_ids in (
        ...
        categories: list[str] = []
        for rule_id in [
            part.strip() for part in str(rule_ids).split(";") if part.strip()
        ]:
...
def _rules_are_simple(rules: pd.DataFrame) -> bool:
    ...
    has_other_keys = rules[non_fingerprint_cols].notna() & (
        rules[non_fingerprint_cols] != ""
    )
...
def _fast_apply_rules(transactions: pd.DataFrame, rules: pd.DataFrame) -> pd.DataFrame:
    ...
    merged["payee_selected"] = merged["payee_single"].where(
...
def _target_lineage_ids(row: Mapping[str, object]) -> tuple[str, ...]:
    ...
    for candidate in [
        ...
        *bank_identity.extract_bank_txn_ids_from_memo(
            row.get("target_memo", row.get("memo", ""))
        ),
        *card_identity.extract_card_txn_ids_from_memo(
            row.get("target_memo", row.get("memo", ""))
        ),
    ]:
...
def _build_target_suggestions_pandas(
    ...
    if _rules_are_simple(rules_pd):
        out = _fast_apply_rules(out, rules_pd)
    else:
        applied = rules_mod.apply_payee_map_rules(pl.from_pandas(out), rules).to_pandas()
        options = _build_options_from_applied(applied, rules_pd)
    ...
    columns = TARGET_SUGGESTION_COLUMNS + [
        col for col in optional_columns if col in out.columns
    ]
...
def _source_transaction_record(row: dict[str, object]) -> dict[str, object]:
    ...
    transaction["source_system"] = (
        _optional_text(transaction.get("source_system")) or "source"
    )
    transaction["transaction_id"] = _optional_text(
        transaction.get("transaction_id")
    ) or _optional_text(transaction.get("ynab_id"))
    transaction["parent_transaction_id"] = _optional_text(
        transaction.get("parent_transaction_id")
    ) or _optional_text(transaction.get("transaction_id"))
    transaction["account_name"] = _optional_text(
        transaction.get("account_name")
    ) or _optional_text(transaction.get("source_account"))
...
def _prepare_review_source_rows(source_df: pl.DataFrame) -> pl.DataFrame:
    ...
    source_work = (
        ...
        .with_columns(
            ...
            .alias("date_key"),
            pl.coalesce(
                [nonempty("raw_text"), nonempty("description_raw"), pl.lit("")]
            ).alias("memo"),
            pl.when(source_type == "bank")
    ...
    if source_work.select(pl.col("fingerprint").eq("").any()).item():
        raise ValueError(
            "Source data missing fingerprint values; run fingerprinting first."
        )

    prepared_pl = (
        source_work.with_columns(
...
def _default_source_context(
    *,
    ...
    return {
        ...
        "source_context_category_id": "",
        "source_context_category_name": review_model.normalize_category_value(
            source_category
        ),
        "source_context_matching_split_ids": "",
    }
...
def _institutional_candidate_pairs(
    ...
    pairs = (
        ...
        .with_columns(
            pl.col("_exact_import_match")
            .any()
            .over("source_row_id")
            .alias("_import_match_by_source")
        )
        .filter(~pl.col("_import_match_by_source") | pl.col("_exact_import_match"))
    ...
    pairs = (
        ...
        .with_columns(
            pl.col("_exact_memo_lineage_match")
            .any()
            .over("source_row_id")
            .alias("_memo_lineage_by_source")
        )
        .filter(
...
def build_review_rows(
    ...
    *,
    ...
    transaction_id_expr = (
        lambda account, date, outflow, inflow, fingerprint, raw_text: pl.struct(
    ...
    matched_pl = (
        pairs_pl.with_columns(
            normalized_category_expr("source_category_current").alias(
                "_source_category_norm"
            ),
            ...
            (
                (
                    ...
                    & bool_text("ynab_cleared").is_in(["cleared", "reconciled"])
                )
            ).alias("_matched_cleared"),
            transaction_id_expr(
                text("account_name"),
                text("source_date"),
                pl.col("outflow_ils"),
        ...
        .with_columns(
            ...
            pl.coalesce([text("ynab_account"), text("account_name")]).alias(
                "account_name"
            ),
            ...
            pl.coalesce([text("fingerprint"), text("ynab_fingerprint")]).alias(
                "fingerprint"
            ),
            ...
            pl.coalesce([text("source_account"), text("account_name")]).alias(
                "source_account"
            ),
            pl.coalesce([text("ynab_account"), text("account_name")]).alias(
                "target_account"
...
def main() -> None:
    ...
    parser.add_argument(
        "--profile", default="", help="Workflow profile (for default map paths)."
    )
