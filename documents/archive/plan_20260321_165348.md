...
- Current Pilates bank dry-run status:
  - 7 existing uncleared YNAB bank rows are exact `stamp+clear` sync candidates
  - 1 existing uncleared YNAB bank row needs nearby-date manual review (`2025-12-22`, 120 ILS)
  - 2 existing uncleared YNAB bank rows currently look like stale orphans (`2025-11-17`, 200 ILS and `2026-01-12`, 500 ILS)

In progress:
- Pilates bank sync/reconcile is mid-stream:
  - sync candidates and uncleared leftovers are now visible
  - the exact sync updates have not yet been executed in the live budget in this branch session
  - reconciliation should resume only after the uncleared leftovers are triaged
...
1. Finish Pilates bank sync + reconcile
- Execute the currently planned Pilates bank sync updates against the live budget.
- Review the uncleared-YNAB triage output and manually resolve the three remaining exceptions:
  - one nearby-match candidate (`2025-12-22`, 120 ILS)
  - two likely stale/manual orphan rows (`2025-11-17`, 200 ILS and `2026-01-12`, 500 ILS)
- Rerun bank reconciliation after the uncleared set is in an acceptable state.
