...
1. Reopen `family / 2026_04_14` review on the rebuilt artifact and verify the transfer review slice:
   ...
   - internal `Bank Leumi -> Opher x9922` transfer rows should no longer claim that a real category is required when both sides are on-budget, even in grouped mode and even in a session that previously had stale account metadata
   - grouped `Accept group` on internal `Transfer : Opher x9922` rows should no longer crash or report `missing target category`
   ...
   - unresolved bank-side `לאומי ויזה` rows should now default to their mapped transfer payees when the suffix can be derived from the bank row
   - if resuming from the existing reviewed artifact, confirm whether stale unresolved blanks like ref `0425898` need an explicit reviewed-artifact refresh because resume still preserves the old blank selections
