...
Fourth hostile audit (Pass 4) complete — readability and clarity from a newcomer's perspective. Pass 4 walked the entire codebase as a first-time reader would, starting from README and following breadcrumbs into the source code. Found 17 items (2 HIGH, 5 MEDIUM, 10 LOW). See `documents/hostile_audit_report.md` Pass 4 section.

Key findings:
- No architectural overview anywhere (data-flow diagram, module-to-stage mapping)
...
- Dual-column `payee_selected`/`target_payee_selected` pattern never explained in code
- Scripts carry business logic with `sys.path` hacks

Current focus:
- Triage Pass 4 findings for implementation priority
...
Pass 4 FIX LIST:
1. HIGH Architecture — Add architectural overview (data-flow diagram, module-to-stage mapping)
2. MEDIUM Organization — Add module grouping or orientation to `__init__.py`
...
9. MEDIUM Documentation — Comment the dual-column `payee_selected`/`target_payee_selected` fallback
...
17. LOW Documentation — Link `review_app_workflow.md` from README
