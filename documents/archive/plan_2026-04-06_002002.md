...
Step 4 is now functionally complete on the branch.

Immediate next steps:
1. do a final manual sanity pass in the live app against representative reviewed data before branch closeout:
   - exercise `Create split`, `Edit split`, `Save split`, and `Cancel`
   - confirm resume/save/load behavior feels correct on real reviewed artifacts
2. if no issues appear in that manual pass, prepare the branch for Step 4 review/merge
3. only add browser automation later if the manual pass uncovers UI behavior that Streamlit `AppTest` cannot cover reliably
