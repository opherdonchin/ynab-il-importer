...
- `build-context-review` still only resolves the active run's declared normalized sources
  - staged previous card snapshots under `derived/previous_max/` and `derived/previous_leumi_card/` are only consumed later by `reconcile-card-cycle`
  - this leaves a workflow gap: a later run can reach card reconciliation with missing historical card rows that were never surfaced for review/import
...
- Family April 14 card closeout exposed the carryforward gap directly:
  ...
  - the active fix should make review build carry forward staged previous-card snapshots for the same card accounts before reconciliation time
...
1. Fix review-build card carryforward:
   - append staged previous card snapshots for the context's card accounts when building review
   - keep active-run sources authoritative by dropping duplicate source transaction ids after concatenation
   - cover both `previous_max` and `previous_leumi_card`
2. Rebuild and verify `family / 2026_04_14`:
   - previously blocked April-cycle card rows should surface in ordinary review before reconciliation
   - already settled YNAB history should still stay out of review by default
3. Finish Family April 14 closeout after the rebuilt review/upload:
   - bank sync/reconcile
   - card sync/cycle reconcile
4. Run the same carryforward-aware review path for the remaining April 14 contexts as needed:
   - Pilates bank + Leumi card
   - Aikido category/account parity
