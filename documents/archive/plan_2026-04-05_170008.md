...
2. extend upload prep so committed target-side split edits are exploded into prepared upload rows and assembled back into split payloads correctly
3. add targeted upload-path tests for:
   - multi-line saved splits
   - one-line collapsed saves
4. keep iterating in small committed slices with `documents/plan.md` updated before each commit
