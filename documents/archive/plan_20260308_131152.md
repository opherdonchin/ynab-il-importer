...
- Existing reviewed decisions need to be reconciled onto a rebuilt `proposed_transactions.csv` because bank/card overlap dedupe now removes duplicate debit-card rows before review.
...
- Upload flow is now the main implementation focus; there is still no YNAB transaction-create client or upload script in the repo.
