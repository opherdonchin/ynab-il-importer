...
- completed working-view slice:
  - [review_working_view](src/ynab_il_importer/review_app/state.py#L161) now builds an explicit Polars working view from the live edited review dataframe
  - that working view carries explicit filter/helper columns for primary state, row kind, blocker label, suggestion label, map-update label, search text, and split-derived display fields
  - [filtered_row_indices_from_view](src/ynab_il_importer/review_app/state.py#L319) now drives the main app filter path from that working view
  - [app.py](src/ynab_il_importer/review_app/app.py#L794) now computes and stores that working view as part of derived app state
...
3. reduce the remaining pandas-heavy paths that still sit outside the working view
   - widget staging and row-edit application
   - grouped summary/default calculations
   - component/blocker precomputations
   - save/change state comparisons where that can be done safely
4. push helper logic further toward dataframe-native expressions
   - filtering
   - search
   - grouping summaries
   - split badges and split detail support
5. keep split handling read-only in the app for now
   - display yes
   - editing deferred to Step 4
6. after the flat review model settles, update the longer-form split plan document to match the new Step 3 direction
