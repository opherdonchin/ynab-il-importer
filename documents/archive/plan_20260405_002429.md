...
- completed adapter-island slice:
  ...
  - [apply_row_edit](src/ynab_il_importer/review_app/state.py#L1080), [apply_to_same_fingerprint](src/ynab_il_importer/review_app/model.py#L56), and [apply_review_state](src/ynab_il_importer/review_app/validation.py#L500) now accept Polars-facing inputs and return updated frames instead of relying on in-place pandas mutation as the only model
  - [apply_competing_row_resolution](src/ynab_il_importer/review_app/model.py#L100) now returns the updated frame together with touched indices, which isolates its imperative conflict-resolution logic cleanly behind a narrower boundary
  - [reconcile_reviewed_transactions](src/ynab_il_importer/review_reconcile.py#L108) now accepts Polars input and only drops into pandas inside the reconciliation implementation
  - focused review/app/reconcile tests now cover both pandas and Polars entrypoints for these hard functions
...
2. push more helper logic onto the explicit Polars working view
   - keep the persisted review artifact mostly flat
   - keep binary/categorical helper columns for app filtering on the working view
   - keep arbitrary payee/category text filtering on the underlying text columns instead of exploding those into binary columns
3. keep removing defensive schema guessing from app helpers
   - prefer direct known review columns over fallback alias-selection helpers
   - keep schema normalization at the app boundary instead of spreading it through `state.py`
4. reduce the remaining pandas-heavy paths that still sit outside the working view
   - widget staging and row-edit application still run through adapter-island functions
   - grouped summary/default calculations that still depend on pandas row slices
   - component/blocker precomputations that still materialize pandas series before becoming filter labels
   - save/change state comparisons where that can be done safely
5. push helper logic further toward dataframe-native expressions
   - filtering
   - search
   - grouping summaries
   - split badges and split detail support
6. treat pandas as an internal implementation detail only in the hard islands
   - `apply_row_edit`
   - `apply_to_same_fingerprint`
   - `apply_competing_row_resolution`
   - `apply_review_state`
   - `apply_review_state_best_effort`
   - `reconcile_reviewed_transactions`
7. keep split handling read-only in the app for now
   - display yes
   - editing deferred to Step 4
8. after the flat review model settles, update the longer-form split plan document to match the new Step 3 direction
