...
- `family / 2026_06_06` is ready for human review after a source-boundary mapping fix:
  ...
  - reviewed artifact: `113` rows, `21` reviewed, `112` upload decisions, `1` no-decision row
...
1. Continue review for `family / 2026_06_06` only from the fresh review app at `http://localhost:8504`.
2. Save the Family reviewed artifact when the remaining rows are settled.
3. Rerun `context-run-status family 2026_06_06` and confirm only expected upload/report artifacts are missing.
