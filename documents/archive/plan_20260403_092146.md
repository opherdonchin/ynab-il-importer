...
Close out Step 1 cleanly and prepare for Step 2 of split-transaction support on the `handle_splits` branch.
...
Hold Step 1 steady and stop before Step 2 implementation.

That means:
1. keep the Step 1 migration green and performant on representative real data
2. finish the detailed Step 2 plan before writing Step 2 code
3. make the next implementation slice explicit enough that it can be reviewed first
...
- Step 2 planning is drafted and ready for review before implementation
...
- starting Step 2 implementation before the YNAB split upload/update policy is reviewed
- under-specifying split verification, especially if YNAB response detail is shallower than expected

## Next Step

Pause for review of the detailed Step 2 plan in `documents/handle_splits_implementation_plan.md`.

Once approved, begin Step 2 with the lowest-risk first slice:
- refactor `ynab_api.py` so the current top-level and exploded YNAB dataframes are explicit projections from the canonical constructor
- add projection tests proving that current download behavior stays stable
- then move the regular upload path onto a transaction-level assembly helper before adding split payloads
