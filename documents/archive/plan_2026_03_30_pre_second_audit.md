...
1. Run another hostile audit against the current committed branch state
2. Triage any new findings into:
   - must-fix before merge
   - good follow-up but not blocking
   - rejected / false positive
3. If the audit is clean or all blocking findings are resolved, prepare the branch for human review
4. After merge, resume Aikido forward updates on `main`
