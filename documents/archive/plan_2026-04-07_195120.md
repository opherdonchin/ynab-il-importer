...
2. Implement Slice 2 of the context workflow refactor:
   - use the new context config to drive run-tag normalization safely from declared source files only
   - keep `x0602`-style wrong-source inclusion from reappearing
3. Implement Slice 3:
   - add a context-driven review-build entrypoint that resolves normalized source artifacts, YNAB artifact, payee map, and output paths from context + run tag
4. During the resumed review, watch the remaining ambiguous groups for cases where coarse fingerprints, especially BIT-style transfers, still hide meaningful distinctions.
5. Once the April 2 review is stable again, promote the corrected rebuilt/reconciled artifact set into `data/paired/2026_04_01/`.
