...
- extend `verify_upload_response(...)` and related response summaries for split-create transactions
- add explicit unsupported handling and messaging for existing split edits before touching the review UI
- keep JSON as the primary dry-run payload preview in `scripts/prepare_ynab_upload.py`
- then decide whether a small live YNAB smoke test is worthwhile for split-create verification before moving to Step 3
