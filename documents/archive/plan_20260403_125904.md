...
- update the review builders to write canonical review Parquet artifacts by default
- keep any flat review CSV as an explicit compatibility/debug projection rather than the authoritative artifact
- then refactor `upload_prep.py` and `scripts/prepare_ynab_upload.py` to consume canonical review artifacts directly
