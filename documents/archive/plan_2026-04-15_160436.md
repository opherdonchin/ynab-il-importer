...
- Family April 14 review exposed a transfer-category regression:
  ...
  - the editor now clears stale `None` transfer categories when a real category is required, so these rows no longer look falsely complete`r`n  - group headers, row details, and category dropdown options now suppress `None` whenever a real category is required
...
1. Reopen `family / 2026_04_14` review on the fully patched app and verify that off-budget transfer groups now show blank category defaults, no `None` category summary, and the category-required note.
