...
- Live cross-budget fix manifests and corrected local rerun artifacts were generated:
  ...
  - curated rerun:
    ...
    - ambiguous rows = `0`
    - proposal mapping quality = `23 unique`, `2 none`
...
- The current live cross-budget blockers are now small and explicit:
  - Family-side transaction corrections needed:
    - `2025-11-02` `Bituach Leumi` inflow should leave `Pilates`
    - `2025-11-05` `Bit fees` should leave `Pilates`
    - `2025-11-06` `גלית כלפון מזומן` cash row is the wrong sign in Family
    - `2025-12-07`, `2025-12-22`, `2026-01-01`, `2026-02-17`, `2026-03-10` should move from Investments to Pilates/`Transfer : Pilates Leumi`
  - Pilates-side transaction correction needed:
    - `2026-02-09` `דנה` `200` inflow in `In Family` looks like a mistaken or duplicate entry
  - after those corrections, only two live proposal rows still need plain review:
    - `google one dublin`
    - `הפקדות קופג`
...
- The cross-budget Pilates flow now needs review/curation rather than new architecture:
  - apply the Family/Pilates direct fixes from the new manifests in live YNAB
  - refresh the Family/Pilates snapshots after those live edits
  - rerun the cross-budget proposal flow against refreshed live data
  - review the two remaining `match_status=none` rows and any newly surfaced edge cases
...
- Apply the direct ledger fixes listed in:
  - `data/paired/pilates_cross_budget_live/family_direct_fix_candidates.csv`
  - `data/paired/pilates_cross_budget_live/pilates_direct_fix_candidates.csv`
- Refresh the live snapshots and confirm the live rerun behaves like the curated local rerun:
  - `unmatched_target = 0`
  - `ambiguous = 0`
  - most proposal rows default automatically
- Review the remaining non-default proposal rows and only then prepare/upload any missing Pilates `In Family` rows.
