...
1. implement the split-mutation core around one transaction struct at a time:
   - read one current transaction
   - replace or normalize its `splits`
   - rebuild the touched working row
   - recompute `changed` for touched rows only
2. add the modal split editor UI:
   - `Create split`
   - `Edit split`
   - `Add line`
   - `Remove line`
   - `Save split`
   - `Cancel`
3. keep split-save validation centralized and incremental:
   - show running amount difference while editing
   - enforce schema/artifact invariants on save
   - refresh cached validation state only for affected rows/components
4. extend reconcile and upload prep so committed split edits survive resume and produce the correct target-side upload payloads
