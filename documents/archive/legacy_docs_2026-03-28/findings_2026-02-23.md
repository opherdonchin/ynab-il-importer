...
`src/ynab_il_importer/io_max.py` now outputs normalized fields including:
...
- `parse-max` no longer takes `--account-name`.
- `parse-leumi` no longer takes `--account-name`.
...
2. Re-parse card + leumi with mappings in place and verify account alignment with YNAB.
