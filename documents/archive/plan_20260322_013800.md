...
- The cross-budget Pilates flow now needs review/curation rather than new architecture:
  - inspect `data/paired/pilates_cross_budget_bootstrap/ambiguous_matches.csv`
  - decide which rows from `data/paired/pilates_cross_budget_bootstrap/payee_map_candidates.csv` belong in `mappings/pilates/payee_map.csv`
...
- Review the bootstrap ambiguous buckets and unmatched target rows.
- Decide which bootstrap payee-map candidates should be merged into `mappings/pilates/payee_map.csv`.
