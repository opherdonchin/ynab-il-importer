...
Keep Step 2 pointed at the canonical review artifact instead of letting flat review rows become a second long-lived model.
...
3. make review input/output canonical, with flat translation only inside the review app boundary
4. re-point upload prep from flat review rows to canonical review artifacts
...
Continue Step 2 at the review/upload boundary:
- refactor `upload_prep.py` and `scripts/prepare_ynab_upload.py` to consume canonical review artifacts directly
- keep any flat review CSV as an explicit compatibility/debug projection rather than the authoritative artifact
- then decide whether the cross-budget builder should gain richer canonical nested snapshots before Step 3
