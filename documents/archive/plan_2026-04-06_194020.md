...
Step 4 is complete on the branch.

Immediate next steps:
1. do one last quick live-app sanity pass on the refreshed demo/reviewed data before branch closeout:
   - confirm valid refreshed demo data still opens cleanly
   - confirm deliberately stale demo data now fails immediately with the expected guidance
   - confirm existing-split save, invalid split-total save, and corrected split-total save all behave cleanly in the modal
   - confirm saved splits immediately switch the row to `Edit split`, preserve modal seed values, and show split amount summaries in the detail table
   - confirm split summaries and full-category split editing read clearly in the UI after removing the unstable modal checkbox
   - confirm cancel followed by quit/reload/save does not resurrect the split editor
   - confirm split amount entry feels natural now that the modal accepts plain text amounts
2. if that pass is clean, prepare the branch for Step 4 review/merge
