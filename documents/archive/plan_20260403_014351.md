...
- real-data note:
  - a representative Family `2026_04_01` builder run was attempted but did not finish within the command timeout window, so Step 1 correctness looks strong from tests, while operational runtime/performance should still be treated as a follow-up concern
...
1. Capture the intentional Step 1 boundaries clearly:
...
2. Decide whether to treat Step 1 as complete and move to Step 2, or do a dedicated performance-focused follow-up on the real-data builders first.
...
Prepare the Step 1 closeout:
- summarize the migrated producer/consumer paths
- call out the intentional CSV compatibility boundaries
- separate correctness progress from the still-open operational runtime question
