...
- This means the remaining cross-budget work is operational rather than architectural:
  - execute the prepared `25`-row upload when ready
  - then refresh into a new dated Family/Pilates snapshot window that starts from the last Family reconciliation and runs through today
...
- The cross-budget Pilates flow is now ready for execution on the current historical window:
  - the local reviewed/upload-prep artifacts are clean
  - no further mapping work is currently required for the saved-snapshot round
  - the next meaningful validation will be a fresh dated snapshot window after this historical upload is complete
...
- Keep the applied live fixes represented by:
  - `data/paired/pilates_cross_budget_live/family_direct_fix_candidates.csv`
  - `data/paired/pilates_cross_budget_live/pilates_direct_fix_candidates.csv`
- Treat the current curated historical round as ready:
  - `matched_existing = 20`
  - `proposed_missing = 25`
  - `unmatched_target = 0`
  - `ambiguous = 0`
  - reviewed/upload-prep artifacts are clean
- Execute the prepared historical upload when ready, then start a new dated Family/Pilates snapshot window from the last Family reconciliation through the current date.
