...
4. fail fast when any normalized review/build input would come from flat CSV instead of canonical parquet
...
- completed canonical-input guard slice:
  - [build_proposed_transactions.py](scripts/build_proposed_transactions.py#L139) now only accepts canonical transaction inputs for proposal building
  - CSV paths are treated only as locators for required canonical parquet sidecars
  - flat normalized CSV artifacts remain export/debug helpers, but they are no longer acceptable fallback processing boundaries when the canonical parquet sidecar is missing
