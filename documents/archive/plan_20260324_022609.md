...
- Full cross-budget balance parity is still not complete, but the gap is much smaller and more explicit:
  ...
  - cached full verify currently blocks on one explicit pre-window unmatched target/base row:
...
- The cross-budget Pilates flow has moved from proposal/upload into reconciliation hardening:
  ...
  - the remaining work is to classify the last legacy/base mismatch cleanly enough that the final full-balance verify pass can succeed
...
- Keep reconciliation packets as the saved run artifact for future cross-budget work:
  - source/category export or source snapshot
  - target YNAB snapshot
  - cached source month report
  - matched/unmatched/ambiguous artifacts
  - reviewed/upload artifacts
  - explicit bootstrap/base exception ids when accepted
- Next cross-budget task:
  - resolve or explicitly document the remaining `250.00` legacy/base difference
  - then rerun the cached full verify pass
