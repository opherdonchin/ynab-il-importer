...
- Pilates card boundary is now documented; we still need to execute the full boundary-forward monthly reconcile sequence in live YNAB.
...
1. Pilates boundary-forward card execution
...
- Apply transition reconciliation consecutively across the clean window (month-to-month) and execute only after each dry-run report validates.
- Keep `2025-12-07` as an optional earlier clean boundary only if we deliberately decide to include the late-2025 itemized rows.
