...
- Review app now supports `--in`, `--out`, `--categories`, and `--resume` CLI args; help documented via `-- --help`.
- Added `--app-help` to show app-specific CLI usage (Streamlit `--help` remains global).
