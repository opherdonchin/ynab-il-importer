...
2. add an explicit Polars working view for the app
   - keep the persisted review artifact mostly flat
   - derive binary/categorical helper columns for app filtering
...
3. push helper logic further toward dataframe-native expressions
   - filtering
   - search
   - grouping summaries
   - split badges and split detail support
4. keep split handling read-only in the app for now
   - display yes
   - editing deferred to Step 4
5. after the flat review model settles, update the longer-form split plan document to match the new Step 3 direction
