...
- Review UI ergonomics under live use, now focused on remaining grouped-review bugs and transfer-specific mapping gaps.
- Review UI quit behavior is now explicit, but still needs real-user validation in a live Streamlit session.
...
- Upload flow is still not implemented in code; there is currently no YNAB transaction-create client or upload script.
...
- Rebuild `outputs/proposed_transactions.csv` after source-overlap dedupe changes.
- Reconcile/port existing review decisions into the rebuilt reviewed file.
- Ensure payee selected for all rows, and category selected for all non-transfer rows.

4) Map updates
- Generate `outputs/map_updates.csv` from reviewed data.

5) Upload flow
- Implement deterministic import-id generation and YNAB transaction-create client.
- Build upload-prep script from reviewed transactions.
- Dry-run against current reviewed data, then verify idempotent re-run behavior before any real upload.
