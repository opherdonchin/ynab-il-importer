...
1. Execute the Family upload from the canonical reviewed-artifact path and verify the YNAB response.
2. Re-run Family bank sync/reconcile after upload so the remaining post-anchor unresolved rows reflect true live-state gaps rather than pre-upload state.
3. Re-run Family card sync/reconcile after upload so the remaining unmatched current rows reflect true live-state gaps rather than pre-upload state.
4. Decide whether [reconcile_card_payment_transfers.py](scripts/reconcile_card_payment_transfers.py) should be converted to the new context/canonical path or removed in favor of the main card-cycle reconciliation flow.
5. Once Family closeout is stable end to end, move on to Pilates and then Aikido.
