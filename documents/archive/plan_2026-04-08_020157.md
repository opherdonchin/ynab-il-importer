...
1. Execute the Family upload from the newly working canonical reviewed-artifact path and verify the YNAB response.
2. Refactor [src/ynab_il_importer/bank_reconciliation.py](src/ynab_il_importer/bank_reconciliation.py) and its scripts onto canonical normalized Parquet + Polars-first boundaries, then verify Family bank sync/reconcile.
3. Add explicit normalization for `data/raw/previous_max/...` statement files before reconciliation.
4. Refactor [src/ynab_il_importer/card_reconciliation.py](src/ynab_il_importer/card_reconciliation.py) and its scripts onto canonical normalized Parquet + Polars-first boundaries, then verify Family card sync/reconcile.
5. Replace the remaining active [workflow_profiles.py](src/ynab_il_importer/workflow_profiles.py) dependencies in upload/sync/reconcile entrypoints with context + run-tag driven scripts.
