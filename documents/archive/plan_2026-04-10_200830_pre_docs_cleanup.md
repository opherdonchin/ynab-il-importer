...
- focused rules/pairing cleanup is now complete:
  - [ynab_api.py](src/ynab_il_importer/ynab_api.py) category dataframe helpers now return `pl.DataFrame`
  - [safe_types.py](src/ynab_il_importer/safe_types.py) no longer carries the dead pandas flag-series helper
  - [export.py](src/ynab_il_importer/export.py) now writes either pandas or Polars dataframes at the CSV boundary
  - [upload_prep.py](src/ynab_il_importer/upload_prep.py) category lookup helpers and `categories_df` upload-prep path are Polars-first
  - [transaction_io.py](src/ynab_il_importer/artifacts/transaction_io.py), [pairing.py](src/ynab_il_importer/pairing.py), and [rules.py](src/ynab_il_importer/rules.py) now use Polars in their active paths
  - immediate callers and tests were cut over in:
    - [bootstrap_pairs.py](scripts/bootstrap_pairs.py)
    - [prepare_ynab_upload.py](scripts/prepare_ynab_upload.py)
    - [download_ynab_categories.py](scripts/download_ynab_categories.py)
    - [map_updates.py](src/ynab_il_importer/map_updates.py)
    - [app.py](src/ynab_il_importer/review_app/app.py)
  - full suite is green again: `pixi run pytest tests/ -q` -> `300 passed`
...
- Pilates `2026_04_01` build now tolerates preserved same-category live YNAB splits:
  - [review_schema.py](src/ynab_il_importer/artifacts/review_schema.py) no longer rejects multi-line splits solely because all lines share one category
  - [build_proposed_transactions.py](scripts/build_proposed_transactions.py) now drops non-Parquet `pl.Object` transaction payload columns from the matched-pairs artifact before writing it
  - `pixi run build-context-review -- pilates 2026_04_01` now succeeds:
    - `24` rows in `data/paired/2026_04_01/pilates_matched_pairs.parquet`
    - `4056` rows in `data/paired/2026_04_01/pilates_proposed_transactions.parquet`
...
## Reconciliation Polars Migration

See [reconcile_polars_migration.md](reconcile_polars_migration.md) for the full design.

Status:
- Slice 1 (`review_reconcile.py`): **complete** — pure-Polars rewrite (no pandas import), all 7 tests pass
- Slice 2 (`bank_reconciliation.py` frame prep + resolution loops): **complete** — all functions updated
  to Polars / list[dict]; report DataFrames remain pandas for caller compatibility; all 14 tests pass
- Slice 3 (bank resolution loops): **merged into Slice 2**
- Slice 4 (`card_reconciliation.py`): **deferred, pending audit**
...
1. Move on to Pilates workflow for `2026_04_01`:
   ...
   - `pixi run build-context-review -- pilates 2026_04_01`  — done
   - Review and upload Pilates transactions from `data/paired/2026_04_01/pilates_proposed_transactions.parquet`
