...
5. compute `changed` only when a row is edited, while initializing it to `False` for fresh reviews and preserving it on resume
6. only then restart Step 4 split-editor implementation
...
The next implementation run is a three-stage redesign that finishes the review-model boundary before Step 4 resumes.

Immediate next steps:
1. write the 4-transaction review-artifact redesign into the long-form split plan
2. implement the new persisted review schema and IO:
   - top-level review/control fields
   - `source_current`
   - `target_current`
   - `source_original`
   - `target_original`
3. implement one centralized flat working projection for the app and upload path
4. move `changed` onto explicit initialization/persistence plus row-update-only recomputation
5. only then restart Step 4 split-editor implementation
