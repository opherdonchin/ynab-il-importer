...
- completed post-manual-pass polish slice:
  - [review_app/app.py](src/ynab_il_importer/review_app/app.py#L1) now falls back from blank top-level `fingerprint` to source/target fingerprint and then transaction id when building grouped view keys, so grouped review no longer disappears on sparse or stale demo data
  ...
  - [tests/test_review_app.py](tests/test_review_app.py#L1) now covers blank-fingerprint grouped fallback, split category summary text, and the demo builder's `review_v4` target-split fixture shape
...
1. do one last quick live-app sanity pass on the refreshed demo/reviewed data before branch closeout:
   - confirm grouped view renders expected groups
   - confirm split summaries and modal category toggles read clearly in the UI
