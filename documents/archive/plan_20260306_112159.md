...
In progress:
- Narrow `subject_payment` handling so `תשלום` maps to Subject only when it is the sole meaningful token.
...
1) Refine subject_payment handling
- Map to Subject only when `תשלום` is the only meaningful token.
- Re-run normalize → match → group to confirm reduced false positives.

2) Curate fingerprint map
