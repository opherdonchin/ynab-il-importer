...
1. Review remaining direct authoritative transaction CSV reads and decide which ones still need Step 1 compatibility loaders.
2. Run equivalence-focused verification on representative workflows before calling Step 1 complete.
3. Capture any Step 1 limits that remain intentional, especially category-source and review-row CSV compatibility boundaries.
...
Do an explicit Step 1 sweep:
- identify any remaining direct transaction CSV reads that should move behind the loader
- run representative workflow checks
- decide whether Step 1 is complete or whether one more compatibility slice is needed
