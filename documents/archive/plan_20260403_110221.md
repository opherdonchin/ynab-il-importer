...
- Step 2 planning has been revised against the official YNAB split API model and is ready for review before implementation
...
- starting Step 2 implementation before the revised single-constructor/category-extraction design is reviewed
...
- consolidate `ynab_api.py` to one canonical transaction constructor
- remove `category_transactions_to_dataframe(...)`
- add category extraction that preserves parent transaction objects
- then migrate one real consumer, ideally `scripts/reconcile_cross_budget_balance.py`, before moving upload prep onto transaction-level assembly
